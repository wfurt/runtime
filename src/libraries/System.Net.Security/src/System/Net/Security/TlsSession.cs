...
#pragma warning restore SYSLIB0039
        ...
        public SslPolicyErrors AcceptWithDefaultValidation()
        {
            ...
            // 10.0 servicing branch does not expose SslStream.VerifyRemoteCertificateCore.
            // Callers that need full SslStream-compatible validation must use
            // SetRemoteCertificateValidationResult with their own verdict.
            throw new PlatformNotSupportedException(
                "AcceptWithDefaultValidation is not available on this servicing branch. " +
                "Use SetRemoteCertificateValidationResult to record an externally-computed verdict.");
        }
            ...
            out int bytesWritten)
        {
            ...
            // 10.0 PAL: DecryptMessage(ctx, buffer, out offset, out count) decrypts in place
            // and points at the plaintext slice inside the same buffer. Map to the multi-arg
            // form used by the rest of this method.
            int decBytesWritten = 0;
        ...
        private static TlsOperationStatus TryDrainBufferedPlaintext(Span<byte> plaintext, out int bytesWritten)
        {
            bytesWritten = 0;
            // 10.0 PAL DecryptMessage signature requires a non-empty ciphertext buffer; the
            // drain-without-input path used on main is not available on this servicing branch.
            // Callers will simply re-enter with fresh ciphertext via the regular Decrypt path.
            return TlsOperationStatus.WantRead;
        }
